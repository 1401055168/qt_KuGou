...
void Lyric::analyzeLrcContent(const QByteArray &KlcData,const QString filedir)//爆力解析内容
{
    ...
    foreach (QString linestr, list)
    {
        ...
        QString getTimestr=strlist.value(0);//歌词时间
        QString getLrcStr=strlist.value(1);//歌词内容

        QString time= getTimestr.remove("[").split(',').value(0);
      //  QString maskdur=getTimestr.remove("[").split(',').value(1);
        QStringList itemList= getLrcStr.split('<');//歌词行列表
        ...
        m_lrcwordsmap.insert(time.toInt(),wordsmap);//时间-歌词
